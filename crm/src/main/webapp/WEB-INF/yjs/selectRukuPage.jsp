		...
		<div class="row">
			<div class="col-sm-12">
				<div class="ibox float-e-margins">
					...
					<input type="text" id="t1" name="goodsName" placeholder="请输入商品名称，或商品类别" style=" width: 400px;height: 30px;font-size: 16pt; ">
   					&nbsp;
						...
						<table class="footable table table-stripped toggle-arrow-tiny"
							...
							<tbody>
				                   <c:forEach items="${p.list}" var="k">
									...
									<td><input type="checkbox" name="c1" class="c1" value="${k.cgXqId}"></td>
										<td>${k.cgXqId }</td>
										<td>${k.cgdId}</td>
										<td>${k.cpId}</td>
										<td>${k.cpNumber }</td>
										...
										<td>${k.remark}</td>
										<td>${k.gsId}</td>									
										<td><fmt:formatDate value="${k.ltime}"
												pattern="yyyy-MM-dd HH:mm:ss" />
										</td>
										<td><a href="goupdate.do?cgdId=${k.cgdId }">修改</a>
											/ <a
											href="delete.do?cgdId=${k.cgdId}"onclick="return confirm('是否确定删除？')">删除</a>
											</td>
											</tr>
								</c:forEach>
							</tbody>
							<tfoot>
								<tr>
									...
									target="_self"
										href="rukulistPage.do?pageNum=${p.firstPage }">首页</a>
										<a target="_self" href="rukulistPage.do?pageNum=${p.prePage }">上一页</a>
										<a target="_self" href="rukulistPage.do?pageNum=${p.nextPage }">下一页</a>
										<a target="_self" href="rukulistPage.do?pageNum=${p.lastPage }">尾页</a>
	...
	$(document).ready(function(){
		...
		$("#deleteSelect").click(function(){
			...
			for(i=0;i<myArr.length;i++){
				...
				if(isdelete){
					location.href="cgdDeleteSelect.do?"+a;
				}
